...
const INSPECTEUR_URL = adresseIp + "/user/findallInspecteur";
const DELETE_USER_URL = adresseIp + "/user/delete";

const Inspecteurs = ({ navigation }) => {
  const [isLoading, setIsLoading] = React.useState(true);
  const [data, setData] = React.useState("");
  const createTwoButtonAlert = (id) =>
    Alert.alert("", "هل ترغب بحذف هذا المتفقد ؟", [
      {
        text: "إلغاء",
        onPress: () => console.log("إلغاء"),
        style: "cancel",
      },
      { text: "حذف المتفقد", onPress: () => fetchDeleteInspecteur(id) },
    ]);

  function fetchInspecteur() {
    axios
      .post(INSPECTEUR_URL)
      .then((response) => {
  ...
  function fetchDeleteInspecteur(id) {
    axios
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerText}>قائمة المتفقدين</Text>
        </View>
            ...
            {data.map((inspector, index) => (
              <View style={styles.friendListItem} key={index}>
                <View style={styles.friendListItemLeft}>
                  <TouchableOpacity
                    style={styles.delete}
                    onPress={() => createTwoButtonAlert(inspector.id)}
                  >
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => navigation.navigate("editUser",{
                      userId: inspector.id,
                      unom: inspector.nom,
                      uprenom: inspector.prenom,
                      uemail:inspector.email,
                      uphone: inspector.telephone,
                      upassword:inspector.password,
                      uecole: inspector.ecole
                    })}
                  >
                ...
                <View style={styles.friendListItemCenter}>
                  <Text style={styles.friendListItemText}>
                    {inspector.nom} {inspector.prenom}
                  </Text>
                </View>
